...
let driftMngr = {
  ...
  driftEnabled: function() {
    this.driftModeEnabled = true;
    mp.game.graphics.notify("DriftMode ~g~enabled");
  },

  driftDisabled: function () {
    this.driftModeEnabled = false;

    if (this.isDrifting) {
      this.onDriftEnded(consts.DriftEndReason.DriftModeDisabled);
    }

    if (mp.players.local.vehicle) {
      mp.players.local.vehicle.setReduceGrip(false);
    }

    if (dr.timebars) {
      dr.timebars.show(false);
    }

    mp.game.graphics.notify("DriftMode ~r~disabled");
  },

  pulse: function () {
    if (!this.driftModeEnabled) {
      return;
    }
  ...
  pulseDrift: function () {
    if (this.driftModeEnabled && this.isDrifting) {
      const vehicle = mp.players.local.vehicle;
